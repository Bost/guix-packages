(define-module (bost gnu packages emacs-xyz)
  ...
  #:use-module (bost guix build spacemacs-utils) ; program-file->package
  #:use-module (bost guix build cli-utils) ; emacs-cli-utils-service
...
;; (define-public emacs-magit
;;   (let ((commit "be8f9bac84141124cb5be7eb5e31cac7fcdeaae2")
;;         (revision "0"))
;;     (package
;;       (name "emacs-magit")
...
;;          (file-name (git-file-name name version))
;;          (sha256
;;           (base32 "0jz6jsafm7cx8nnpwrykga08lir93vkz7xndkbr70jnnxb9wpp1m"))))
;;       (build-system emacs-build-system)
;;       (arguments
;;        (list
;;         ;; #:tests? #t
...
(define-public emacs-tweaks
  (let ((commit "bed6994ba7edb06d9da84c7d983b5714b822f107")
        (revision "0"))
    (package
      (name "emacs-tweaks")
      (version (git-version "0" revision commit))
      (source
       (origin
         ...
         (file-name (git-file-name name version))
         (sha256
          (base32 "0rbv0ikhlq3ihkrmfzm10zdaq7b1bls93l4vms24rcjmmpik6wgh"))))
      (build-system emacs-build-system)
      (propagated-inputs
       (list
        emacs-copy-sexp
      ...
      (synopsis
       "Emacs tweaks for Evil/Spacemacs, Clojure/CIDER, and Scheme development")
      (description
       "Emacs utilities for Evil/Spacemacs, Clojure/CIDER, and Scheme
development.")
      (license license:gpl3+))))
...
(define-public emacs-spacemacs
  (let ((commit "686875a30dbc378a2d9dfbf086e39fb211f2bc03")
        (revision "0"))
    (package
      ...
      (source
       (origin
         ...
         (file-name (git-file-name name version))
         (sha256
          (base32 "1s4qbqz4m5h5h2wk8rv6ibkvivqgcxvalc0m7d84rvxsp3vpr903"))))
      (build-system emacs-build-system)
      (arguments
       (list
        #:tests? #f
...
;; (define* (program-file->package idx
;;                                 #:key
;;                                 name
;;                                 (version "1.0.0")
;;                                 (synopsis "")
...
;;                                 (license #f))
;;   (let* [(program (list-ref (emacs-cli-utils-service (assoc-ref %build-inputs "spacemacs")) idx))
;;          (prg-file (cadr program))]
;;     (package
...
;;                   (src #$source)
;;                   (dst (string-append bin #$(string-append "/" name)))]
;;               (format #t "### copy-file ~a ~a\n" src dst)
;;               (mkdir-p bin)
;;               (copy-file src dest)
;;               (chmod dst #o555)))))
;;       (propagated-inputs emacs-spacemacs)
;;       (synopsis synopsis)
;;       (description description)
;;       (home-page home-page)
;;       (license license))))

;; (define-public emacs-spacemacs-launcher         (program-file->package 9))
;; (define-public emacs-spacemacs-editable-init-el (program-file->package 10))
;; (define-public emacs-spacemacs-pkill-server     (program-file->package 11))


;; (use-modules (guix derivations) (guix store))
;; (define daemon (open-connection))
;; (build-derivations daemon (list (package-derivation daemon emacs-spacemacs-launcher)))
(define-public emacs-spacemacs-launcher
  (package
    (inherit emacs-spacemacs)
    (name "emacs-spacemacs-launcher")
    (source #f)
    (build-system trivial-build-system)
    (inputs `(("spacemacs" ,emacs-spacemacs)))
    ...
    (arguments
     (list
      ...
      '(
        ...
        (gnu packages docker)
        (gnu packages mail)
        (gnu packages perl-web)
        (gnu packages openkinect)
        ...
        (gnu packages dns)
        (gnu packages tls)
        (gnu packages specifications)
        (gnu packages prometheus)
        (gnu packages golang-maths)
        ...
        (gnu packages maths)
        (gnu packages fortran-check)
        (gnu packages fortran-xyz)
        ...
        (bost srfi-1-smart)
        (bost utils)
        (bost guix build cli-utils)
        (guix build utils) ; mkdir-p, copy-file, chmod
        )
      #:builder
      #~(begin
          (use-modules
          ...
          (define* (emacs-cli-utils-gexp)
            ((comp
              car
              (partial map (comp
                            (partial apply service-file-gexp)
                            (partial append (list
                                             #:exec-fun 'exec-foreground
                                             #:extra-modules '((bost guix build emacs-common)))))))
             (list
              (list #:utility "x" #:fun 'create-launcher #:params spguix
                    #:init-dir (spacemacs-site-lisp-dir spacemacs-store-path)))
             ))

          (let* [(spacemacs-store-path (assoc-ref %build-inputs "spacemacs"))
                 (bin (string-append #$output "/bin"))
                 (src #$source)
                 (dst (string-append bin #$(string-append "/" name)))
                 ]
            ;; (format #t "### bin ~a\n" bin)
            ;; (format #t "### spacemacs-store-path ~a\n" spacemacs-store-path)
            ;; (format #t "### init-dir ~a\n" (spacemacs-site-lisp-dir spacemacs-store-path))
            ;; (format #t "### copy-file ~a ~a\n" src dst)
            (format #t "### gexp ~a\n" (emacs-cli-utils-gexp))
            ;; (mkdir-p bin)
            ;; (copy-file src dest)
            ;; (chmod dst #o555)
            ))))))
