(define-module (bost guix build emacs-common)
...
#!/usr/bin/env -S guile \\
-L ./guix/common -L ./guix/home/common -e (bost guix build emacs-common) -s
!#
...
;; (define* (f a #:optional (b (+ 1 a))) (list a b))

(define* (create-init-cmd profile #:optional (init-dir (get-src profile)))
"
...
"
  (cmd->string
   (list
    (which-emacs)
    (format #f "--init-directory=~a" init-dir)
...
"
  (when trace
    ...
    (format #t "~a #:init-dir      ~a ; ~a\n" f (pr-str-with-quote init-dir)      (test-type init-dir))
  ...
  (let* [(elements (list #:trace #:verbose #:ignore-errors
    ...
    (apply exec-system*-new
           ...
           #:trace trace
           #:verbose verbose
           (list "pkill" "--echo" "--full" (create-init-cmd params init-dir)))))
...
\"src/bost/guix/build/cli-common.scm\" \"--create-frame\")

(create-launcher #:params \"spguix\" \"src/bost/guix/build/cli-common.scm\" \"--create-frame\")
"
  (when trace
    (format #t "~a   args          ~a ; ~a\n" f (pr-str-with-quote args)          (test-type args))
    (format #t "~a #:trace         ~a ; ~a\n" f (pr-str-with-quote trace)         (test-type trace))
  ...
  (let* [(elements (list #:trace #:verbose #:ignore-errors
         ...
         (filtered-args (remove-all-elements args elements))
         (init-cmd (create-init-cmd params init-dir))]
    ((comp
